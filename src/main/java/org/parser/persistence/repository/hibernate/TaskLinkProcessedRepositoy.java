...
public class TaskLinkProcessedRepositoy extends AbstractRepo {
    ...
    public List<TaskLink> getNotProcessedLink(int i) {
        return (List<TaskLink>) sessionFactory.getCurrentSession().createQuery("from task_link p where p.systemType = :id and p.processed = false order by p.qty_entity ").setParameter("id", i).list();
    }
}